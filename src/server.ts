...
document.addEventListener('click',function(e){var b=e.target.closest('[data-dl]');if(!b)return;e.preventDefault();var d=b.getAttribute('data-dl').split(':');var type=d[0],id=d[1],qAttr=d[2]||'';var btn=b;btn.disabled=true;var txt=btn.textContent||btn.innerText;(function(){var q=qAttr;var s=document.getElementById('dc-q');if(s&&s.value)q=s.value;return fetch('/api/dl/'+type+'/'+id+(q?'?quality='+q:''),{method:'POST'})})().then(function(r){if(!r.ok)throw new Error(r.status+' '+r.statusText);return r.json()}).then(function(t){if(t.error){alert(_l==='en'?'Download error: '+t.error:'下载失败: '+t.error);btn.disabled=false;btn.textContent=txt;return;}alert(_l==='en'?'Queued: '+t.label:'已加入下载队列: '+t.label);setTimeout(function(){location.reload()},2000)}).catch(function(e){alert(_l==='en'?'Download failed: '+e.message:'下载失败: '+e.message);btn.disabled=false;btn.textContent=txt})});
...
function vlPage(videos: string[], title: string, backUrl: string, lang: Lang, dlBtns?: string, pageInfo?: string, pgHtml?: string): string {
  ...
  <div class="bento-p"><div class="bento-b stagger">${videos.map(v => `<div class="li" style="cursor:default;flex-wrap:wrap">
    ...
    <div class="li-act btn-grp" style="align-self:flex-start">
      ...
      <button class="btn btn-s btn-xs" data-dl="video:${v}:720p">${I.dl}</button>
    </div>
...
function checkStorage(cfgIn?: any): string {
  ...
  try {
    ...
    if (c.protocol === "webdav") {
      // Use rclone for reliable WebDAV connectivity test
      const webdavUrl = remote.replace(/\/$/, "");
      cmd = `rclone lsd :webdav: --webdav-url="${webdavUrl}" --webdav-user="${c.username || 'none'}" --webdav-pass="${c.password || ''}" 2>&1 | head -5; echo "EXIT:$?"`;
    ...
    // Check for rclone EXIT code first (WebDAV)
    if (c.protocol === "webdav") {
      const exitMatch = out.match(/EXIT:(\d+)/);
      if (exitMatch && exitMatch[1] === "0") return "ok";
      // Extract meaningful error from rclone output
      const errLine = out.split("\n").filter(l => l.includes("Failed") || l.includes("Error") || l.includes("error") || l.includes("404") || l.includes("401") || l.includes("403") || l.includes("Couldn't")).join("\n");
      return (errLine || out).slice(0, 600) || "rclone failed";
    }
