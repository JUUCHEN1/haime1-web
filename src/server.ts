...
document.addEventListener('click',function(e){var b=e.target.closest('[data-dl]');if(!b)return;var d=b.getAttribute('data-dl').split(':');var type=d[0],id=d[1],qAttr=d[2]||'';var btn=b;btn.disabled=true;var txt=btn.textContent||btn.innerText;(function(){var q=qAttr;var s=document.getElementById('dc-q');if(s&&s.value)q=s.value;return fetch('/api/dl/'+type+'/'+id+(q?'?quality='+q:''),{method:'POST'})})().then(function(r){if(!r.ok)throw new Error(r.status+' '+r.statusText);return r.json()}).then(function(t){if(t.error){var m=document.getElementById('dl-msg');if(m){m.textContent=_l==='en'?'Download error: '+t.error:'下载失败: '+t.error;m.className='toast toast-error';m.style.display='block';setTimeout(function(){m.style.display='none'},3000)}else alert(_l==='en'?'Download error: '+t.error:'下载失败: '+t.error);btn.disabled=false;btn.textContent=txt;return;}var m=document.getElementById('dl-msg');if(m){m.textContent=_l==='en'?'Queued: '+t.label:'已加入队列: '+t.label;m.className='toast toast-ok';m.style.display='block';setTimeout(function(){m.style.display='none'},3000)}else alert(_l==='en'?'Queued: '+t.label:'已加入队列: '+t.label);setTimeout(function(){location.reload()},2500)}).catch(function(e){var m=document.getElementById('dl-msg');if(m){m.textContent=_l==='en'?'Download failed: '+e.message:'下载失败: '+e.message;m.className='toast toast-error';m.style.display='block';setTimeout(function(){m.style.display='none'},4000)}else alert(_l==='en'?'Download failed: '+e.message:'下载失败: '+e.message);btn.disabled=false;btn.textContent=txt})});
